...
export class Scheduler {
    ...
    public getFutureWTasks(timestamp: number): Promise<WrappedTaskInterface[]> {
        return this.storage.getWrappedTasksByExecutionTimestamp(timestamp, timestamp + this.timeFuturePeriod);
    }

    public doLoop(timestamp: number) {
        this.getFutureWTasks(timestamp)
            .then((scheduledWTasks) => {
                for (const tsk of scheduledWTasks) {
                    const timeToWait = tsk.task.meta.executionTimestamp - timestamp;
